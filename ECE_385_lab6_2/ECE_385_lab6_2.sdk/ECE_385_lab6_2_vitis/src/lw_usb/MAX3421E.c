...
void MAXreg_wr(BYTE reg, BYTE val)
{
	...
	XSpi_Transfer(&SpiInstance, temp, NULL, 2);
...
BYTE *MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE *data)
{
	...
	BYTE temp[nbytes + 1];
	temp[0] = reg + 2;
	memcpy(temp + 1, data, nbytes);
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	XSpi_Transfer(&SpiInstance, temp, NULL, nbytes + 1);
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return data + nbytes;
}
...
BYTE MAXreg_rd(BYTE reg)
{
	...
	BYTE temp[2] = {reg, 0};
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	XSpi_Transfer(&SpiInstance, temp, temp, 2);
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return temp[1];
}
...
BYTE *MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE *data)
{
	...
	temp[0] = reg;
	XSpi_SetSlaveSelect(&SpiInstance, 1);
	XSpi_Transfer(&SpiInstance, temp, temp, nbytes + 1);
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	memcpy(data, temp + 1, nbytes);
	return data + nbytes;
}
...
if (action)
{ // turn on by setting GPOUT0
tmp |= bmGPOUT0;
}
else
{ // turn off by clearing GPOUT0
tmp &= ~bmGPOUT0;
}
...
{ // bus reset causes connection detect interrupt
...
{ // bus event is either reset or suspend
